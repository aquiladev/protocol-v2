...
contract PriceFeed is IPriceFeed {
  ...
  constructor(
    ...
    lastGoodPrice = _scaleChainlinkPriceByDigits(
  ...
  function _bothOraclesSimilarPrice(
    ...
    uint256 minPrice =
      (scaledBandPrice < scaledChainlinkPrice) ? scaledBandPrice : scaledChainlinkPrice;
    uint256 maxPrice =
      (scaledBandPrice >= scaledChainlinkPrice) ? scaledBandPrice : scaledChainlinkPrice;
  ...
  function _getCurrentBandResponse() internal view returns (BandResponse memory bandResponse) {
    try bandOracle.getReferenceData(bandBase, bandQuote) returns (
      ...
      bandResponse.timestamp = lastUpdatedBase < lastUpdatedQuote
        ? lastUpdatedBase
        : lastUpdatedQuote;
    ...
    returns (ChainlinkResponse memory chainlinkResponse)
  {
    ...
    try chainlinkOracle.latestRoundData() returns (
      ...
      chainlinkResponse.answer = _scaleChainlinkPriceByDigits(
        uint256(answer),
        chainlinkResponse.decimals
      );
    ...
    returns (ChainlinkResponse memory prevChainlinkResponse)
  {
    ...
    try chainlinkOracle.getRoundData(_currentRoundId - 1) returns (
      ...
      prevChainlinkResponse.answer = _scaleChainlinkPriceByDigits(
        uint256(answer),
        _currentDecimals
      );
